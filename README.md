...
| File | Role |
|------|------|
| `app.py` | Entry point: loads audio path (resolved from **this file’s directory**, not the shell’s current folder), calls transcribe + summarize, saves `.txt` files, prints the summary. |
...
If the code lives in a subfolder (for example `audio-summarizer/`), use **that** folder’s virtualenv and `requirements.txt`.

1. Put your audio under `samples/` (or edit `audio_path` in `app.py`).
2. Run the script from **any** working directory; paths are anchored to `app.py`:
...
On Windows you can double-click **`audio-summarizer\run.bat`** so it always uses that folder’s `venv` (avoids “module not found” when another Python is default).

or, from anywhere (still use this venv so `imageio-ffmpeg` is available):

```bash
C:\path\to\audio-summarizer\venv\Scripts\python.exe C:\path\to\audio-summarizer\app.py
```
